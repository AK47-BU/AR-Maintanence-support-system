...
When writing about the security architecture in your COMP5074 report, reference:

- **Section 4c.1** — the JWT + bcrypt + RBAC implementation in `middleware/auth.js` and `middleware/rbac.js`
- **Section 4c.2** — Helmet, CORS, rate limiting, input validation in `server.js` and `middleware/validate.js`
- **Section 4c.3** — the audit logging system in `models/AuditLog.js` and the security events endpoint
- **Section 4c.4** — justify why JWT over sessions, why bcrypt over argon2, why MongoDB over SQL for this prototype
